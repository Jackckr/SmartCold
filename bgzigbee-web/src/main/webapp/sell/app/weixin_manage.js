...
function col_cellStyler(value, row) {
    return [
        '<button class="btn" onclick="initConfigForm(\'',row.wxname,'\')">配置</button>'
    ].join("");
}
...
var goDeleteUser = function (uid, username) {
    $.messager.confirm('删除确认', '你确认要<er>删除</er>用户<er>' + username + '</er>吗?', function (r) {
        if (r) {
            $.ajax({
                type: 'GET', url: '../../i/user/deleteUser', data: {"userID": uid}, success: function (data) {
                    reloaddata();
                }
            });
        }
...
//==================================================配置=================================================
//改变user
function changeuser(keyword) {
    $.ajax({
        type: 'post', url: '/i/user/getUserList', data: {rows: 20,page:1, keyword: keyword}, success: function (data) {
            $("#sel_user_slid").combobox("loadData", data.rows);
        }
    });
}

function initConfigForm(nickname) {
...
//保存用户
function saveConfig() {
    $.ajax({url:"/i/useropenid/changeConfig",data:{userId:$("#sel_user_slid").combobox('getValue'),id:$("#id").val()},type:"post",success:function (data) {
...
function user_uplevel() {
    ...
    if (level != oldlevel && id != "") {
        $.post('../../i/user/changeLevel', {'userID': id, 'vipType': level}, function () {
            reloaddata();
        });
    }
}
...
//	var fottol  ={buttons: [{text:'添加用户',iconCls:'user_add',handler:adduser},{text:'编辑用户',iconCls:'user_edit',handler:edituser},{text:'删除用户',iconCls:'user_del',handler:deleteUsers},"-"]};
    var col = [[
        ...
        {field: 'openid', title: 'openID', width: 80, align: 'center', sortable: true, formatter: getUsername},
    ...
    initTable("微信id配置", "main_weixin", "POST", "../../i/wxUser/getUserByFilter", queryParams, '#user_filter', null, col, true, onDblClickRow);
//    objTable.datagrid({singleSelect: true});//设置为单选
}
function getUsername(username) {
    return username ? username : "暂无配置";
}
//======================================================================初始化数据=======================================================
//初始化数据
$().ready(function () {
    ...
    $('#sel_user_auto').combobox({
        onChange: function (val) {
            queryParams.audit = val;
            reloaddata(queryParams);
        }
    });
});
