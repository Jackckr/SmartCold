function gordcdile(sharid) {
    window.location.href = "view/colddetail.html?id=" + sharid;
};
function goshadile(sharid) {
...
function gordclist() {
    ...
    $("#searchdiv").val("");
    window.location.href = "view/coldlist.html?key=" + key;
};
function gosharlist() {
    var key = $("#searchdiv").val().trim();
    window.localStorage.setItem("shdatakey", key);
...
function goshkeylist(em) {
    var key = $(em).attr("value");
    window.localStorage.setItem("shdatakey", key);
    $("#searchdiv").val("");
...
$().ready(function () {
    ...
    function addshhist(key) {//添加历史纪录
        ...
        if (hist) {
            ...
            if (hist.indexOf(":\"" + key + "\",")) {
                $.each(histjson, function (index, item) {
                    if (item.key == key) {
                        i = index;
                        return false;
                    }
