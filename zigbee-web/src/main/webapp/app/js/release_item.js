...
var releaseItem = {
    ...
    changtype:function(_em){
    	  var em=$(_em); $("#item_type_div span").removeClass("outCur");
	       em.addClass("outCur");
	       releaseItem.$scope.typeCode=em.attr("value");
	       releaseItem.$scope.typeText=$(em).text();
...
coldWeb.controller('releaseItem',function($rootScope, $scope, $stateParams, $state, $cookies, $http, $location) {
	...
	$scope.appmode=[{},{tool:[[1,"出货"],[2,"求货"]],rul:"/i/ShareRdcController/shareFreeRelease"}];
	$scope.gocoldShareComment=function(){
		  $state.go('coldShareComment');
	};
    $scope.initpag = function() {
