...
if(window.user==undefined ||window.user==null){var userjson=window.localStorage.lkuser;if(userjson){window.user=JSON.parse(userjson);}}
...
function checktoken(toke){if(window.user==null ){if(toke==undefined){toke=util.getCookie('token');}$.ajax({type:"GET",cache:false,timeout : 5000,dataType:"json",data:{token:toke}, url:ER.root + "/i/user/findUser",success:function(data) {if (data && data.id != 0) {window.user = data;window.localStorage.lkuser=JSON.stringify(data);} else {window.user = null;window.localStorage.lkuser=null;}} });}}
