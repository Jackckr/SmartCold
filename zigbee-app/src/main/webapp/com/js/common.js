...
if(!sessionStorage.lkuser){checktoken();}
...
function setIOS_deltoken(){ $http.get(ER.root+'/i/user/logout');window.user  = null;util.delCookie("token");util.delCookie("lkuser");sessionStorage.removeItem("lkuser");}//同步token
...
function checktoken(toke){if(toke==undefined){toke=localStorage.token;}$.ajax({type:"GET",cache:false,timeout : 5000,dataType:"json",data:{token:toke}, url:ER.root + "/i/user/findUser",success:function(data) {if (data && data.id != 0) {window.user = data;window.localStorage.lkuser=JSON.stringify(data);sessionStorage.lkuser=window.localStorage.lkuser;} else {window.user = null;window.localStorage.removeItem("lkuser");sessionStorage.removeItem("lkuser");}} });}
