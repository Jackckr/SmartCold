var rdc= JSON.parse(localStorage.rdc),pagedata={index:0},mySwiper=undefined;
mui.init();
...
var temp={
	...
	initStorageset:function(){
    	  	 mui.ajax(smartCold + 'i/coldStorageSet/findStorageSetByRdcId', {data: {rdcID: rdc.id},dataType: 'json',  type: 'get',  crossDomain: true,success: function (data) {
    	 	   ...
    	 	   var count=0;
    	 	     $.each(rdc.mystorages, function(i, mystorage){
                        ...
                        success: function (data) {
	                        count++; var oids=[],names=[];
	                        mui.each(data, function(i, temp){oids.push(temp.id);names.push(temp.name);   });
	                        mystorage.oids=oids;mystorage.names=names;
    ...
    inidata:function(mystorage){
    	     if(mystorage.lasttime&&new Date().getTime()-mystorage.lasttime<30000 &&$("#chart_"+mystorage.id+" canvas").length>0){return;}
    	     mui.get(smartCold + 'i/util/getColdAlarmStatus', {oid: mystorage.id}, function(data) {$("#errmsg_" + mystorage.id).html(data.isBlack?"<h4 class='red overTemp'>由于该冷库长时超温,系统自动静默！</h4>":""); }, 'json');
    	 	 $("#cuttval_"+mystorage.id).empty();
    	 	 var endTime = new Date(), startTime = new Date(endTime.getTime() - 1.5 * 60 * 60 * 1000);
    	 	 mui.ajax(smartCold + 'i/temp/getTempByTime', { data: {'oid': mystorage.id,'oids': mystorage.oids, 'names': mystorage.names,'key': 'Temp', "startTime": formatTime(startTime),"endTime": formatTime(endTime)},dataType: 'json',type: 'post',timeout: 10000,traditional: true, success: function (result) {
             var name = result.name;
             var datumTemp = parseFloat(result.startTemperature) + 0.5 * parseFloat(result.tempdiff);//基准温度
             var  xData = [], series = [],tempData = [], tempMap = result.tempMap, isadd = true, ymax=datumTemp,ymin=datumTemp,systime = result.systime;
             ...
             for (var key in tempMap) {
                ...
                if (tempList.length > 0) {
                    for (i = 0; i < tempList.length; i++) {
                        ...
                        if(vo.value>ymax){ymax=vo.value}
                        if(vo.value<ymin){ymin=vo.value}
                    }
            ...
            if (series.length > 0) {
            	...
            	ymin=-20,ymax=20;series= {data:[]};//为了无数据能够显示气泡
     ...
     initchart:function(storageid,ymax,ymin,xData,series){
            ...
            var option = {
		        ...
		        yAxis: {
                    ...
                    min:ymin,
                    type : 'value',
             		axisLabel: {  formatter: function (value, index) { if(value){return parseFloat(value).toFixed(1);     }return 0;}  }
                },
                series: series
            };
...
pwc={//初始化电量模块
	...
	initpwcset:function(){
		mui.ajax(smartCold + 'i/power/findByRdcId',{data:{rdcId:rdc.id},
			dataType:'json',//服务器返回json格式数据
			type:'get',//HTTP请求类型
	...
	initdata:function(powerSet){
		      if(powerSet.lasttime&&new Date().getTime()-powerSet.lasttime<30000 &&$("#chart_"+powerSet.id+" canvas").length>0){return;}
...
water={
	...
	initwaterset:function(){
		mui.ajax(smartCold + 'i/compressorGroup/findByRdcId',{data:{rdcId:rdc.id},
			dataType:'json',//服务器返回json格式数据
			type:'get',//HTTP请求类型
			timeout:10000,//超时时间设置为10秒；
			success:function(data){
	...
	initdata:function(compressorGroup){
		  if(compressorGroup.lasttime&&new Date().getTime()-compressorGroup.lasttime<30000 &&$("#chart_"+compressorGroup.id+" canvas").length>0){return;}
	 ...
	 refdata:function(){
		  if(mySwiper&&mySwiper.pageindex!=pagedata.index){	page.initSwiper(rdc.compressorGroups) }
...
page={
	...
	initpage:function(){
		 console.log(" page: "+pagedata.index)
...
page.initpage();
clearInterval(DiDa);
var DiDa = setInterval(function () { page.initpage();}, 10000);
