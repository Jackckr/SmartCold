	...
	private void SummaryTempWarning(){
		...
		if (SetUtil.isnotNullList(tempWanningCoun)) {
			for (HashMap<String, Integer> hashMap : tempWanningCoun) {
				coldStorageAnalysisList.add(new ColdStorageAnalysisEntity(0, hashMap.get("oid"), "OverTempCount", hashMap.get("longcou"), date));
				coldStorageAnalysisList.add(new ColdStorageAnalysisEntity(0, hashMap.get("oid"), "OverTempTime", hashMap.get("lontime"), date));
			}
