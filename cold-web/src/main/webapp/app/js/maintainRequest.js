coldWeb.controller('maintainRequest', function($rootScope, $scope, $state, $stateParams, $cookies, $http, $location, baseTools) {
	...
	$scope.initdata = function() {
		$http.get('/i/warningMint/getWarningType', {
			params : {
				pid : 0
			}
		...
		$http.get('/i/warningMint/getWarningMintById', {
			params : {
				ids : $scope.warids,
	...
	$scope.swartype = new Array();
	$scope.nwardata = new Array();
	if ($scope.st == 0) { //查看or 审批
		...
		$scope.removerSelect = function(index) {
			$scope.nwardata.pop(index);
		};
		...
		$scope.submit = function() {
			...
			$.ajax({
				...
				url : "/i/warningMint/addMaintenance",
				success : function(data) {
					alert("维修确认单已发送！");
		...
		//查看
		$http.get('/i/warningMint/getMaintenanceByWId', {
			params : {
				wid : $scope.warids
			}
		}).success(function(data) {
		...
		$("#div_maintainRequest input").attr({
			"disabled" : "disabled"
		});
		$scope.tol_advice = function(isreject, status, msg) {
			...
			$http({
				method : 'POST',
				url : '/i/warningMint/rejectMaintenanceByWarId',
				params : {
					isreject : isreject,
					wid : $scope.warids,
					mid : $scope.maintenance.id,
					status : status
				}
			}).success(function(data) {
				if (data) {
